...
def sx_project(tmp_path: Path) -> Path:
    ...
    - 1 fonte ADVPL que usa A1_COD, GetMV(MV_XYZUSED), Pergunte(ABCREL01),
      e a função user U_XYZVALID().
    - Fixtures SX já copiadas.
    """
    src = tmp_path / "src"
    src.mkdir()
    (src / "ABCTEST.prw").write_bytes(
        b"User Function ABCTEST()\n"
        b"  Local cCli := M->A1_COD\n"
        b"  Local cMV  := SuperGetMV('MV_XYZUSED', .F., '01')\n"
        b"  Pergunte('ABCREL01', .T.)\n"
        b"  RecLock('SA1', .T.)\n"
        b"  Replace A1_NREDUZ With MV_PAR01\n"
        b"  MsUnlock()\n"
        b"Return\n"
    )
    (src / "ABCXVALID.prw").write_bytes(
...
class TestLintCrossFile:
    def test_lint_cross_file_detects_sx_rules(
        ...
        # - SX-004 (ABCDEAD pergunta nunca usada)
    ...
    def test_sx005_detects_unused_custom_field(
        ...
        Nenhum aparece no ABCTEST.prw (que só usa A1_COD, A1_NREDUZ, MV_*, ABCREL01),
