...
class TestInstallGeminiSkills:
    def test_installs_all_three_layers_when_signals_present(
        ...
        """~/.gemini/ + .gemini/ projeto → home + project MD + 57 skills."""
        ...
        assert result.installed_skills_count == 57
    ...
    def test_installs_to_agents_skills_when_present(
        ...
        assert len(gemini_files) == 57
        assert len(agents_files) == 57
        assert result.installed_skills_count == 57
        assert result.installed_agents_skills_count == 57
