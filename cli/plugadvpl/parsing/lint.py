...
_CLIENT_PREFIX_RE = re.compile(
    r"^(ABC|MZF|ZZF|U_|ZF|CLI|XX|MT[A-Z]|MA\d|FAT|FIN|COM|EST|CTB|FIS|PCP|MNT)",
    re.IGNORECASE,
)
...
def _check_sec002_user_function_no_prefix(
    ...
    (^[A-Z]{2,4}\\d{2,4}[A-Z_]*$) nem com prefixo de cliente conhecido (ABC, MZF, U_, ZF, ...)
        ...
        findings.append(
            {
                ...
                "sugestao_fix": (
                    ...
                    "adote convenção (e.g., ABC*, ZF*, U_<cliente>*) para evitar colisão."
                ),
            }
        )
    return findings
