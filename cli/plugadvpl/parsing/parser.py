...
# #84: GetAdvFVal("SA1","A1_NOME",...) lê um campo por nome de tabela = read.
_GETADVFVAL_RE = re.compile(r'GetAdvFVal\s*\(\s*["\'](\w{2,3})["\']', re.IGNORECASE)
...
def _extract_tables_from_stripped(
    ...
    for m in _RETSQL_RE.finditer(stripped_keep_strings):  # #81: TCQuery legado
        read.add(m.group(1).upper())
    for m in _GETADVFVAL_RE.finditer(stripped_keep_strings):  # #84: GetAdvFVal
        read.add(m.group(1).upper())
