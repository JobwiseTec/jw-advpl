...
- <project>/.gemini/skills/plugadvpl-<X>/SKILL.md (70 specifics)
...
class InstallResult:
    ...
    installed_skills_count: int  # 0..70 (.gemini/skills/)
...
def install_gemini_skills(project_root: Path, version: str) -> InstallResult:
    ...
    Nota (v0.38.0): o cross-write em ``.agents/skills`` saiu daqui — agora é
    de codex_skills.py (DONO do diretório), evitando colisão de marker entre
    GEMINI_MARKER_PREFIX e CODEX_SKILL_MARKER_PREFIX no mesmo arquivo.
    """
    skipped: list[str] = []
    errors: list[str] = []
