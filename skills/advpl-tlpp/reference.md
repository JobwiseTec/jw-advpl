...
> **Não confundir** com default value na assinatura:
> - **Default na assinatura**: `function f(n := 0)` — `:=` define o valor que `n` assume se o caller omitir.
> - **Named arg na chamada**: `f(n=10)` — `=` passa `10` pro parâmetro formal chamado `n`, independente da posição.

**Operador: `=` (igualdade simples).** Liberado em AppServer **20.3.2.0+** (funções/métodos) e **24.3.1.0+** (`New()` de classes).

```tlpp
function xParams(p1, p2, p3, p4, p5, p6, p7, p8)
return .T.

xParams(p2=b, p1=a, p6=f)         // ordem livre
xParams(a, b, p7=g, p8=h)         // mistura: posicionais primeiro, nomeados depois
```

Regra da fronteira ADVPL: o **caller** precisa estar em `.tlpp`, mas o **callee pode estar em `.tlpp` ou `.prw`** — não há restrição pra função chamada.

Cobertura completa (gotchas, refactor, exemplos REST tlppCore, classe via `New()` com sintaxe canônica TOTVS, anti-padrões posicionais legados) na skill dedicada [[advpl-tlpp-named-params]].

**Fonte oficial:** [Parâmetros Nomeados em TLPP — Hugo Guilherme Gomes / TOTVS Developers](https://medium.com/totvsdevelopers/par%C3%A2metros-nomeados-em-tlpp-ec2211bfe346)
