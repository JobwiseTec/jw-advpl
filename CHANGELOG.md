...
- **`lint <arquivo>` avisa quando o alvo não está no índice ([#118](https://github.com/JoniPraia/plugadvpl/issues/118))** — "sem findings" era ambíguo: arquivo limpo **ou** nem analisado (não indexado). Agora, se o arquivo passado não está em `fontes`, o `lint` imprime um aviso em stderr ("não está no índice — o lint NÃO analisou; rode `ingest`") em vez de parecer "revisado e OK". Evita que um agente marque tarefa como revisada sem revisão. (`arch` já distinguia; `find`/`callers` são busca, onde vazio = não-encontrado é esperado.)
- **Lint `WS-005`: `Return .F.` em endpoint REST notation ([#120](https://github.com/JoniPraia/plugadvpl/issues/120))** — detecta estaticamente o `Return .F.` dentro de `User Function` decorada com `@Get/@Post/...`, que vira **HTTP 500** e descarta o `setStatusCode` (severidade warning). Escopo restrito ao notation (no WSRESTFUL clássico `Return .F.` + `SetRestFault` é o correto → zero FP). Completa o trio do relatório: **REST-010 já existia como `WS-004`** (método inválido no `oRest`) e **REST-012 = `SEC-001`** (`RpcSetEnv` em REST). Achado de uso real (RPO 2510).
...
- **Catálogo de schematics oficiais ([#99](https://github.com/JoniPraia/plugadvpl/issues/99))** — `poui-componentes schematics` lista os **13 generators** oficiais do PO UI (`ng generate @po-ui/ng-templates:po-page-dynamic-table`, `:po-page-login`, `@po-ui/ng-components:po-page-default`, `:sidemenu`, …) por **caso-de-uso**, para a IA recomendar gerar a tela ao invés de montá-la à mão e errar. Catálogo `poui_schematics` (gerado de `projects/{ui,templates}/schematics/ng-generate` do po-angular + caso-de-uso curado, via `build_poui_schematics.py`). Migration 032 (schema v32).
- **Lint POUI-VERSION (versão-aware, [#98](https://github.com/JoniPraia/plugadvpl/issues/98))** — o catálogo PO UI é um snapshot do po-angular (hoje **v21**), mas bindings/props mudam entre majors (ex.: `p-hide-text-overflow` removido do po-table). `poui-lint` agora emite um **aviso POUI-VERSION** quando o `poui_major` detectado do projeto difere do major do catálogo, deixando explícito que os findings de POUI-PROP/POUI-IFACE podem divergir (em vez de dar falso-positivo/negativo silencioso). A versão do catálogo é gravada por `build_poui_catalog.py` (lookup `poui_catalog_meta`, migration 031, schema v31).
...
Fecha o gap do **conteúdo** das tabelas-catálogo (#75, surgido do experimento V3 de reconstrução cega): importa o dump pro índice e cruza com os fontes — cobertura de reconstrução 90% → ~98%.
