...
| **Cursor** | `.cursor/rules/plugadvpl-*.mdc` × 72 | `.cursor/` no projeto |
| **GitHub Copilot** | `.github/copilot-instructions.md` + `.github/instructions/plugadvpl-*.instructions.md` × 72 | `.github/` no projeto |
| **Gemini CLI** | `~/.gemini/GEMINI.md` (home) + `<project>/GEMINI.md` + `.gemini/skills/plugadvpl-*/SKILL.md` × 72 | `~/.gemini/` ou `gemini` no PATH ou `.gemini/` no projeto |
| **Codex CLI** | `.codex/config.toml` + **skills nativas** `.agents/skills/plugadvpl-*/SKILL.md` × 72 (+ `.codex/skills/` legado; + `~/.agents/skills/` global se já existir) | `.codex/`, `.agents/`, ou `codex` no PATH |
...
| `advpl-excel` | **Ler/escrever Excel** — não há função nativa de leitura (CSV+FWFileReader, YExcel, OLE MsExcel); escrita FWMsExcelXlsx/FwPrinterXlsx; **rejeita funções alucinadas** (FWLerExcel/OleAuto/...) |
| `advpl-word` | **Gerar/preencher Word** (carta/contrato/mala direta) — família `OLE_*` + template `.dot`/DocVariable (client-side) e RTF+MemoWrite (server-side); **rejeita funções alucinadas** (MsWord/FWMSWord/FWGeraWord/...) |
...
- **72 skills** (30 knowledge + 42 slash command wrappers), 6 agents especializados (`advpl-analyzer`, `advpl-code-generator`, `advpl-reviewer-bot`, `advpl-impact-analyzer`, `advpl-log-investigator`, `advpl-ini-auditor`), 1 SessionStart hook
...
- **Skills nativas do Codex** em `.agents/skills/plugadvpl-*/SKILL.md` (× 72, *open agent skills standard* — auto-discovery), replicadas em `.codex/skills/` (legado) e em `~/.agents/skills/` (global, só se `~/.agents` já existir). Frontmatter `name: plugadvpl-<X>` + `description`.
